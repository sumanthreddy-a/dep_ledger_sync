...
const ledgerSync = require('./ledegerSync')
...
const getBlock = events => {
  ...
  let url = require('../system/config')
  let finalUrl = `${url.url}/blocks/${block.block_id}`
  axios.get(finalUrl).then((data) => {
    let payload = data.data.data.batches[0].transactions[0].payload

    const base64 = require('base-64')
    const utf8 = require('utf-8')
    var decodedData = base64.decode(payload);
    let aadhar = JSON.parse(decodedData)
    ...
    let ledger = new ledgerSync(blockData)
      ledger.save().then((success)=>{
        // console.log(success)
      })
...
const handleEvent = msg => {
  if (msg.messageType === Message.MessageType.CLIENT_EVENTS) {
    // const base64 = require('base-64')
    // const utf8 = require('utf-8')
    // var bytes = base64.decode(msg.content);
    // var text = utf8.decode(bytes);
    // console.log(text);
    // if(msg.content!=null && msg.content!=''){
    // console.warn('Received message of unknown type:', JSON.parse(msg.content))
    // }
    /*
    let text = JSON.stringify(msg.content)
    let data = Buffer.from(JSON.parse(text).data);
    ...
    */
    ...
    console.warn('Received message of unknown type:', msg.messageType, msg)
  }
}
